...
async function readVaultFile({vaultPath, sharePaths, fileName}) {
    ...
    //For the filename specified, get the metadata for that file
    for (const fileEntry of manifest.files) {
        if (fileEntry.name !== fileName) continue
    ...
    const content = Buffer.from(decryptedFile).toString("utf8")
    sodium.memzero(decryptedFile)
